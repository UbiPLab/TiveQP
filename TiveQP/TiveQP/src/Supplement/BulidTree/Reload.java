...
public class Reload {
    ...
    public String path = "F:\\TiveQP\\Supplement\\2w_5km_2\\";
    //    public String path = "F:\\TiveQP\\Supplement\\1k\\";
    //    public String path = "C:\\E\\8\\";
    public int round = 0;

    public TreeCon.TiveTreeNode BuildTree(String[][] nums){
        //  空树
        if(nums.length==0)
            return null;
        //  叶子节点
        if (nums.length==1){
            TreeCon.TiveTreeNode root = new TreeCon.TiveTreeNode(nums);
            root.flag = round++;
            root.height = 1;
            return root;
        }
...
//        root.flag = new Random().nextInt(2000000);
    ...
    public long initNode(TreeCon.TiveTreeNode root , int ibf_length , String[] Keylist , int rb , List<Long> sizeofCS) throws Exception {
        long time = 0;
        if(root==null)
            return time;
...
//        System.out.println(root.address + "is ok");
     ...
     * @throws Exception
     */
    public long initLeafNode(TreeCon.TiveTreeNode leafnode , int ibf_length , String[] Keylist , int rb, List<Long> sizeofCS ) throws Exception {
        ...
        sizeofCS.add(RamUsageEstimator.sizeOf(leafnode.LCS)+RamUsageEstimator.sizeOf(leafnode.TCS));
...
//        System.out.println("leafnode "+leafnode.flag +" is ok");
     ...
     * @throws
     */
    public long initMidNode(TreeCon.TiveTreeNode midnode, int ibf_length , String[] Keylist , int rb , List<Long> sizeofCS) throws Exception {
        long start = System.currentTimeMillis();
        //  创建 ibf
        Byte[][] twinlist = new Byte[2][ibf_length];
//        for (int i = 0; i < ibf_length; i++) {
...
//        System.out.println(end_read-start_read);
        ...
        sizeofCS.add(RamUsageEstimator.sizeOf(midnode.LCS)+RamUsageEstimator.sizeOf(midnode.TCS));
...
//        System.out.println("midnode "+midnode.flag +" is ok");
    ...
    //  层次遍历
    public void levelOrder(TreeCon.TiveTreeNode root){
        if(root==null)
            return;
        Queue<TreeCon.TiveTreeNode> queue = new LinkedList<>();
        queue.offer(root);
        while(!queue.isEmpty()){
            TreeCon.TiveTreeNode node = queue.poll();
            if (node.height > 1)
                node.showData();
            if(node.left!=null)
     ...
     * @param node
     */
    public void behindOrder2(TreeCon.TiveTreeNode node){
        if(node==null)
            return;
        Stack<TreeCon.TiveTreeNode> s = new Stack<>();

        TreeCon.TiveTreeNode curNode; //当前访问的结点
        TreeCon.TiveTreeNode lastVisitNode; //上次访问的结点
        curNode = node;
        lastVisitNode = node;
...
//                System.out.print(curNode.val+" " );
    ...
    //  后序遍历
    public void postOrder(TreeCon.TiveTreeNode root){
        if(root==null)
            return;
        postOrder(root.left);
