...
public class LocationCoding {
    ...
    //  2km  25 11
    //  1km  50 12
    public final int time = TiveQP_NoType_parameters.time;
    public final int bitsize = TiveQP_NoType_parameters.bitsize;
    ...
    public String[] City_Encoding_Complement(String city_name, double lat, double lng){
        ...
        if (num_lat == 0){
            if (num_lng == 0){
                ...
                String[] n1 = IndexElementEncoding.range(bitsize,0,encode-time-1);
                String[] n2 = IndexElementEncoding.range(bitsize,encode-time+2,encode-1);
                String[] n3 = IndexElementEncoding.range(bitsize,encode+2,encode+time-1);
                String[] n4 = IndexElementEncoding.range(bitsize,encode+time+2,time*time);
            ...
            if (num_lng == 0){
                ...
                //  上边
                String[] n1 = IndexElementEncoding.range(bitsize,0,encode-time-2);
                String[] n2 = IndexElementEncoding.range(bitsize,encode-time+2,encode-2);
                String[] n3 = IndexElementEncoding.range(bitsize,encode+2,time*time);
                ...
                String[] n1 = IndexElementEncoding.range(bitsize,0,encode-time-2);
                String[] n2 = IndexElementEncoding.range(bitsize,encode-time+2,encode-2);
                String[] n3 = IndexElementEncoding.range(bitsize,encode+2,encode+time-2);
                String[] n4 = IndexElementEncoding.range(bitsize,encode+time+2,time*time);
                String[] result = new String[n1.length+n2.length+n3.length+n4.length];
                for (int i = 0; i < n1.length; i++) {
                    result[i] = n1[i];
