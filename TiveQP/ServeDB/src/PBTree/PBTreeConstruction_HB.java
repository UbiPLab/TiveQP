...
public class PBTreeConstruction_HB {
    public String path = "G:\\TiveQP\\ServeDB\\10w_1km_2\\";
...
////        leafnode.HB = MyUtil.HashFounction.mdinstance.digest(toPrimitives(twinlist));
//        int group = ibf_length/ segment_len;
//        leafnode.HB = new byte[group][];
//        for (int i = 0; i < group; i++) {
//            Byte[] segment = new Byte[segment_len];
//            for (int j = 0; j < segment.length; j++) {
//                segment[j] = twinlist[i* segment_len +j];
//            }
//            leafnode.HB[i] = MyUtil.HashFounction.mdinstance.digest(toPrimitives(segment));
//        }
...
//        ReadFiledata.saveArray(toPrimitives_B(HB),leafnode.address_HB);
...
//        System.out.println("leafnode "+leafnode.flag +" is ok");
    ...
    public long initMidNode(PBTreeConstruction.PBNode midnode , int ibf_length , String[] Keylist ) throws Exception {
        long start = System.currentTimeMillis();
        midnode.address =path+ "leafnode "+midnode.flag+".txt";
