...
public class PBTreeConstruction_IF {
    public String path = "H:\\TiveQP\\ServeDB\\10w_1km\\";
...
//        public byte[] HL;
//        public byte[][] HB;
//        public PBTreeConstruction.PBNode left;
//        public PBTreeConstruction.PBNode right;
...
//        leafnode.YCS = new Owner().OwnerType_Complement(leafnode.data[0][0]);
        ...
        for (int i = 0; i < type.length; i++) {
            insert(twinlist,type[i],Keylist,segment_len);
        }

        //  插入 location 数据
        for (int i = 0; i < location.length; i++) {
            for (int j = 0; j < location[i].length; j++) {
                insert(twinlist,location[i][j],Keylist,segment_len);
            }
        }
        ...
        for (int i = 0; i < Time.length; i++) {
            for (int j = 0; j < Time[i].length; j++) {
                insert(twinlist,Time[i][j],Keylist,segment_len);
            }
        }
...
//        }
        ...
        for (int i = 0; i < ibf_length; i++) {
            twinlist[i] = together(twinlist_left[i],twinlist_right[i]);
            twinlist[i] = together(twinlist_left[i],twinlist_right[i]);
        }
        //  准备数据
//        //  type
