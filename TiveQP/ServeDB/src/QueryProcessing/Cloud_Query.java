...
//    public List<Double> list_height = new ArrayList<>();
    ...
    public void Query_Tree(PBTreeConstruction.PBNode root , String[][] T1, String[][] T2, String[][] T3, int ibf_length ,
                           List<Long> time_query,List<Long> time_proof,List<Long> time_proof_all, List<String> result, int num_k,List<byte[]> list_HL,List<Double> list_height,List<Cloud_Query.Proof_MT> mts, List<Cloud_Query.Proof_UMT> umts,
                           List<Cloud_Query.Proof_Mid_ALL> mids, List<Cloud_Query.Proof_UNN> unns) throws Exception {
//        long time = 0;
        if (check_time>=num_k){
